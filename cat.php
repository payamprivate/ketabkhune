...
<?php
	...
	$catagory=$_GET['cat'] ;
	$query="SELECT * FROM `books` WHERE `cat` LIKE '$catagory'";
	...
	<table class="_form" border="0" cellpadding="5" cellspacing="4" width="100%">
		...
		<colgroup valign="middle"></colgroup>
				...
				for ($i=0; $i < $rows ; $i++){
					...
					if ( !$status[$i]==0 ){
                       ...
                       <? if ( isset($_SESSION["username"]) ) { ?>
                        <center> موجود <br/>
                        <a href="<? echo 'cat.php?cat='.$catagory; ?>&amanat=<? echo $serial[$i]; ?>" ><input type="button" name="submita" class="text_only has_text button" value="امانت کتاب" /></a></center></td>
                        <? } else { ?>
            ...
            <?
				...
				echo "<br/><center><b> هیچ کتابی در پایگاه داده ها این دسته ثبت نشده است </b></center>";
			}
			?>
